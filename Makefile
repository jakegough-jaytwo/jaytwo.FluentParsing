...
DOCKER_TAG?=jaytwo_fluentparsing
...
build: restore
	dotnet build ./jaytwo.FluentParsing.sln

test: unit-test
  
unit-test: build
	rm -rf out/testResults
	dotnet test ./test/jaytwo.FluentParsing.Tests \
		--results-directory ../../out/testResults \
		--logger "trx;LogFileName=jaytwo.FluentParsing.Tests.trx"

pack:
	rm -rf out/packed
	cd ./src/jaytwo.FluentParsing; \
